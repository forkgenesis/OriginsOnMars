#### OriginsOnMars - это модпак на Forge 1.18.2 с Origins с технико-магической тематикой.
Развивайте техническую ветвь с *ComputerCraft*, *Create*, *Mekanism*, *Industrial_* или магическую ветвь с *Ars Nouveau*, *Enchanted: Witchcraft* и *Malum*.\
Модпак создавался только для сервера OriginsOnMars, но распространяется по лицензии Creative Commons Zero.

# Установка!!!
1. Скачать ZIP-архив последнего релиза модпака (справа, с зелёной пометкой "Latest").
2. Извлечь файлы из архива в папку вашей игры: `.minecraft` либо `.tlauncher/legacy/Minecraft/game`.

При повторной установке удалить файлы с прошлой установки! 

#### Полный список модов в самом низу!
![OriginsOnMars mod showcase](https://i.imgur.com/foLgale.jpg)

# Список модов
Название | Описание | Зависимости | Сторона
:-:|:-:|:-:|:-:
[Antique Atlas 7.1.1](https://modrinth.com/mod/antique-atlas/version/7.1.1-forge-mc1.18.2) | Добавляет особую карту | Cloth Config API 6.5.102, Architectury API 4.11.93 | Оба
[Architectury API 4.11.93](https://modrinth.com/mod/architectury-api/version/4.11.93+forge) | Библиотека для разработки мультиплатформенных модов | | Оба
[Ars Nouveau 2.9.0]() | Мод на магию, (СПОЙЛЕРЫ) | Curios API 5.0.9.1, Patchouli 71.1 | Оба
[bad packets 0.1.3](https://modrinth.com/mod/badpackets/version/forge-0.1.3) | Библиотека для обмена пакетами между мультиплатформенными модами | | Оба
[Better Compatibility Checker 1.1.21 build.48](https://modrinth.com/mod/better-compatibility-checker/version/build.48+mc1.18.2) | Улучшенная проверка совместимости модпаков | | Оба
[Caelus API 3.0.0.2](https://modrinth.com/mod/caelus/version/1.18.1-3.0.0.2) | Библиотека, имплементирующая поведение элитры для моддеров | | Оба
[Canary 0.2.4](https://modrinth.com/mod/canary/version/mc1.18.2-0.2.4) | Улучшает производительность Minecraft, переписывая такие системы, как физический движок, AI модов, система тиков (форк Lithium) | | Оба
[CC: Tweaked 1.101.3](https://modrinth.com/mod/cc-tweaked/version/1.18.2-1.101.3) | Добавляет программируемые компьютеры и черепашки (форк ComputerCraft) | | Оба
[Cloth Config API 6.5.102](https://modrinth.com/mod/cloth-config/version/ZbWG3eJW) | Библиотека, упрощающая работу модов с конфигами | | Оба
[Clumps 8.0.0+17](https://modrinth.com/mod/clumps/version/t18CfscF) | Группирует XP-орбы вместе | | Сервер
[Connectivity 3.2](https://legacy.curseforge.com/minecraft/mc-mods/connectivity/files/3833738) | Исправляет множество сетевых ошибок и анализирует трафик | Cupboard 1.5 | Оба
[Create 0.5.1.b](https://modrinth.com/mod/create/version/1.19.2-0.5.1.b) | Это Create | | Оба
[Create Crafts & Additions 20230623a](https://legacy.curseforge.com/minecraft/mc-mods/createaddition/files/4605624) | Соединяет кинетическую энергию из Create с электрической | Create 0.5.1.b | Оба
[Create Chunkloading 1.4.0](https://legacy.curseforge.com/minecraft/mc-mods/create-chunkloading/files/4571169) | Добавляет блок, прогружающий чанки около штуковин Create | Architectury API 4.11.93, Cloth Config API 6.5.102 | Оба
[CreativeCore 2.6.17](https://modrinth.com/mod/creativecore/version/2.6.17) | Библиотека CreativeMD | | Оба
[CullLessLeaves Reforged 1.0.5](https://legacy.curseforge.com/minecraft/mc-mods/culllessleaves-reforged/files/4457157) | Пропускает рендер внутренних слоёв листвы | | Клиент
[Cupboard 1.5](https://legacy.curseforge.com/minecraft/mc-mods/cupboard/files/4691988) | Библиотека someaddon | | Оба
[Curios API 5.0.9.1](https://modrinth.com/mod/curios/version/1.18.2-5.0.9.1) | Библиотека для упрощения работы со слотами | | Оба
[Dimensional Threading Reforged (Unofficial) 1.2.8](https://legacy.curseforge.com/minecraft/mc-mods/dimensional-threading-reforged/files/4518078) | Оптимизация работы с несколькими измерениями благодаря многопоточности | | Оба
[Dynamic View 2.8](https://legacy.curseforge.com/minecraft/mc-mods/dynamic-view/files/3814624) | Динамически изменяет дальность прорисовки чанков и сущностей в зависимости от нагрузки на сервер | | Сервер
[Dynmap 3.7 beta 1](https://legacy.curseforge.com/minecraft/mc-mods/dynmapforge/files/4765899) | Добавляет веб-страницу с динамически меняющейся картой сервера | | Сервер
[Easier Sleeping 2.1.3](https://modrinth.com/mod/easier-sleeping/version/1.18.2-2.1.3) | Позволяет пропускать ночь, даже если спит только часть игроков | | Сервер
[Enchanted Witchcraft 1.1.4](https://legacy.curseforge.com/minecraft/mc-mods/enchanted-witchcraft/files/4793875) | Большой магический мод, (СПОЙЛЕРЫ) | GeckoLib 3.0.57, Patchouli 71.1, SmartBrainLib 1.9, StateObserver 1.2.1 | Оба
[Enhanced Block Entities Reforged (Unofficial) 0.8.0](https://legacy.curseforge.com/minecraft/mc-mods/enhanced-block-entities-reforged-unofficial/files/4619590) | Рендерит модели блоков вместо моделей блок-сущностей | Refogium 1.0.12a | Клиент
[Entity Culling 1.6.1](https://modrinth.com/mod/entityculling/version/1.6.1-1.18.2) | Пропускает рендер сущностей, которые не видны игроку | | Клиент
[Farsight 1.9](https://legacy.curseforge.com/minecraft/mc-mods/farsight/files/3810400) | Кэширует чанки в клиенте, позволяя видеть дальше, чем дальность прорисовки сервера | Cupboard 1.5 | Клиент
[FastFurnace 6.0.3](https://legacy.curseforge.com/minecraft/mc-mods/fastfurnace/files/3717883) | Переписывает логику печей, снижая их нагрузку на сервер | Placebo 6.6.7 | Сервер
[Fastload 3.4.0](https://legacy.curseforge.com/minecraft/mc-mods/fastload/files/4706140) | Улучшает механизм генерации мира | | Оба
[FastSuite 3.0.2](https://legacy.curseforge.com/minecraft/mc-mods/fastsuite/files/3755834) | Позволяет серверу быстрее сопоставлять рецепты, используя многопоточность | Placebo 6.6.7 | Сервер
[FastWorkbench 6.1.1](https://legacy.curseforge.com/minecraft/mc-mods/fastworkbench/files/4030000) | Аналогично FastFurnace, снижает нагрузку функций крафта на сервер | Placebo 6.6.7 | Сервер
Fan-Made Discs 2.5.2 | Добавляет кастомные музыкальные диски | | Оба 
[GeckoLib 3.0.57](https://modrinth.com/mod/geckolib/version/3.0.57) | Библиотека, добавляющая поддержку анимации моделей | | Оба
[Get It Together, Drops! 1.3](https://modrinth.com/mod/get-it-together-drops/version/v1.3) | Позволяет кастомизировать объединение упавших предметов на землю | | Сервер
[GraveStone Mod 1.0.2](https://legacy.curseforge.com/minecraft/mc-mods/gravestone-mod/files/3713957) | Создаёт могилки при смерти | | Оба
[ImmediatelyFast Reforged (Unofficial) 1.1.10](https://legacy.curseforge.com/minecraft/mc-mods/immediatelyfast-reforged/files/4447910) | Оптимизирует весь мгновенный рендер, имплементируя кастомный графический буфер | | Клиент
Industrial_ B3.99 | Технический мод с мемами | | Оба
[Just Enough Resources 0.14.2.206](https://modrinth.com/mod/just-enough-resources-jer/version/0.14.2.206) | Добавляет в REI информацию о рудах| REI 8.3.667 | Оба
[Just Zoom 1.0.2-1](https://modrinth.com/mod/just-zoom/version/1.0.2-1-1.18.2-forge) | Приближение на кнопку (Z по умолчанию) | Konkrete 1.6.1 | Клиент
[Konkrete 1.6.1](https://modrinth.com/mod/konkrete/version/1.6.1-1.18-1.18.2-forge) | Библиотека для модов Keksuccino | | Клиент
[Lazy DataFixerUpper 1.0](https://legacy.curseforge.com/minecraft/mc-mods/lazy-dfu-forge/files/3544496) | Небольшой мод для оптимизации DataFixerUpper | | Оба
[Lodestone 1.4.2](https://legacy.curseforge.com/minecraft/mc-mods/lodestone/files/4631609) | Библиотека Lodestar Team | | Оба
[Log Begone 1.0.4](https://modrinth.com/mod/log-begone/version/nYO2EEJH) | Убирает ненужные логи (при удалении, к примеру, блоков) | | Сервер
[Malum 1.5.0.1](https://legacy.curseforge.com/minecraft/mc-mods/malum/files/4631612) | Темно-магический мод, (СПОЙЛЕРЫ) | Curios API 5.0.9.1, Lodestone 1.4.2 | Оба
[Mekanism 10.2.5.465](https://modrinth.com/mod/mekanism/version/10.2.5.465) | Технический мод с упором на энергию, автоматизацию, производство и прогресс | | Оба 
[Mekanism Additions 10.2.5.465](https://modrinth.com/mod/mekanism-additions/version/10.2.5.465) | Аддон на Mekanism, добавляющий мини-мобов, голосовой чат и много дополнительных предметов | Mekanism 10.2.5.465 | Оба
[Mekanism Generators 10.2.5.465](https://modrinth.com/mod/mekanism-generators/version/10.2.5.465) | Аддон на Mekanism, добавляющий генераторы энергии | Mekanism 10.2.5.465 | Оба
[Mekanism Tools 10.2.5.465](https://modrinth.com/mod/mekanism-tools/version/10.2.5.465) | Аддон на Mekanism, добавляющий броню, щиты и инструменты | Mekanism 10.2.5.465 | Оба
[Modern Online Picture Frames 13](https://modrinth.com/mod/modern-online-picture-frames/version/13) | Добавляет возможность вставлять изображения с интернета в рамки | Architectury API 4.11.93, CreativeCore 2.6.17 | Оба
[Oculus 1.6.4](https://modrinth.com/mod/oculus/version/1.18.2-1.6.4) | Форк мода Iris, позволяющий устанавливать шейдеры на Rubidium | Rubidium 0.5.6 | Клиент
OOM Origins 1.4.1.1-unified | Версия Origins специально для сервера OriginsOnMars. Origins - мод на расы, изменяющие геймплей Minecraft | Caelus API 3.0.0.2, Pehkui 3.7.9 | Оба
[Patchouli 71.1](https://modrinth.com/mod/patchouli/version/1.18.2-71.1-forge) | Библиотека для внутриигровой документации | | Оба
[Pehkui 3.7.11](https://modrinth.com/mod/pehkui/version/3.7.11+1.18.2-forge) | Библиотека для управления размерами сущностей | | Оба
[Placebo 6.6.7](https://legacy.curseforge.com/minecraft/mc-mods/placebo/files/4500363) | Библиотека для модов Shadows_of_Fire | | Сервер
[Reforgium 1.0.12a](https://modrinth.com/mod/reforgium/version/1.18.2-1.0.12a) | Библиотека для модов, использующих API рендеринга (форк Indium) | | Клиент
[REI Plugin Compatibilites 8.0.83](https://modrinth.com/mod/roughly-enough-items-hacks/version/8.0.83) | Аддон на REI, позволяющий аддонам на JEI работать с ним | REI 8.0.83 | Оба
[Roughly Enough Items 8.3.667](https://modrinth.com/mod/rei/version/8.3.667+forge) | Добавляет возможность смотреть рецепты внутри игры | Cloth Config API 6.5.102, Architectury API 4.11.93 | Оба
[Rubidium 0.5.6](https://modrinth.com/mod/rubidium/version/0.5.6) | Форк мода Sodium - полностью переписанного движка рендеринга Minecraft | | Клиент
[Rubidium Dynamic Lights 1.3.3](https://legacy.curseforge.com/minecraft/mc-mods/dynamiclights-reforged/files/3817266) | Форк LambDynamicLights, добавляющий в Rubidium динамическое освещение | | Клиент
[Rubidium Extras 1.3.2](https://legacy.curseforge.com/minecraft/mc-mods/magnesium-extras/files/3936081) | Добавляет кучу фич в Rubidium | Rubidium 0.5.6 | Клиент
[Rubidium Occlusion Culling Fix](https://modrinth.com/mod/occlusion-culling-fix-sodium/version/1xe2nuym) | Фиксит баг с невидимыми чанками в Rubidium | Rubidium 0.5.6 | Клиент
[Sit 1.3.3](https://modrinth.com/mod/bl4cks-sit/version/2gEpTXyB) | Позволяет сидеть на ступеньках и полублоках | | Оба
[SmartBrainLib 1.9](https://modrinth.com/mod/smartbrainlib/version/RC9fgR3J) | Библиотека для упрощения работы моддеров с системой мозга | | Оба
[Smooth Boot (Reloaded) 0.0.2](https://modrinth.com/mod/smooth-boot-reloaded/version/mc1.18.2-0.0.2) | Изменяет процесс загрузки Minecraft, ускоряя его | | Клиент
[Sophisticated Backpacks 3.18.65.933](https://legacy.curseforge.com/minecraft/mc-mods/sophisticated-backpacks/files/4808057) | Мод на рюкзаки | Sophisticated Core 0.5.98.458 | Оба
[Sophisticated Core 0.5.98.458](https://legacy.curseforge.com/minecraft/mc-mods/sophisticated-core/files/4808233) | Библиотека для Sophisticated Backpacks | | Оба
[Starlight 1.0.2](https://modrinth.com/mod/starlight-forge/version/1.0.2+1.18.2) | Переписывает движок освещения Minecraft | | Оба
[StateObserver 1.2.1](https://legacy.curseforge.com/minecraft/mc-mods/stateobserver/files/4793861) | Библиотека для наблюдения за изменениями состояния блоков | | Оба
[Storage Drawers 10.2.1](https://legacy.curseforge.com/minecraft/mc-mods/storage-drawers/files/3807626) | Стильный мод на шкафы, в которых можно хранить предметы | | Оба
[TexTrue's Rubidium Options 1.0.8](https://modrinth.com/mod/textrues-rubidium-options/version/1.0.8-mc1.18.2) | Стильные настройки Rubidium | Rubidium 0.5.6 | Клиент
[WTHIT 4.13.6](https://modrinth.com/mod/wthit/version/forge-4.13.6) | Мод на внутриигровую подсказку о блоке, на который смотрит игрок | bad packets 0.1.3 | Оба

